...
def train(rank, world_size, net_params,save_path,training,dataset_path,model_evaluate):
    ...
    # Data initialization
    if training:
        train_set = dataset.MeshDataset(dataset_path, K=10, features=['points'], train=True, augment=['scale','orient'], nb_augments=10, nb_split=16,name="shrec",descriptors=False)
    else:
        train_set = dataset.MeshDataset(dataset_path, K=10, features=['points'], train=False, augment=['scale','orient'], nb_augments=10, nb_split=16,name="shrec",descriptors=False)
    ...
    for j in range(epochs):
        ...
        if rank == 0 and training:
            torch.save(net.state_dict(),f'{save_path}/SHREC_auto{j}')
...
if __name__== "__main__":
    ...
    save_path = f"/home/germainb/PHD_work/MeshConv3D/models/SHREC_auto/SHREC_{date_time}" #Path towards the saved checkpoints of the trained models
    dataset_path = "/home/germainb/PHD_work/Datasets/SHREC11/shrec" #Path towards the source folder of your dataset
